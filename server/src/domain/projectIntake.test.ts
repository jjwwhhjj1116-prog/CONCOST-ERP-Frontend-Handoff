...
test('requires the project intake company to match the selected workspace', () => {
  const actor = {
    personnelId: 'admin-1',
    role: 'SUPER_ADMIN',
    departmentId: 'dept-1',
  ...
  assert.equal(resolveProjectIntakeCompanyScope(actor), 'CON_COST');
  assert.equal(resolveProjectIntakeCompanyScope(actor, 'VIET_QS'), 'VIET_QS');
  assert.equal(resolveProjectIntakeCompanyScope({ ...actor, role: 'PM' }, 'VIET_QS'), null);
  assert.equal(canAccessProjectIntakeCompany('CON_COST', 'CON_COST'), true);
  assert.equal(canAccessProjectIntakeCompany('CON_COST', 'VIET_QS'), false);
  assert.equal(canAccessProjectIntakeCompany('CON_COST', null), false);
});
