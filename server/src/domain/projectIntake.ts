import { z } from 'zod';
import { canAccessCompany, getAllowedCompanyIds } from './companyAccess';
...
export type ProjectIntakeActor = {
  personnelId: string;
  role: string;
  departmentId: string;
  companyId?: string;
...
export function resolveProjectIntakeCompanyScope(
  ...
  return canAccessCompany(allowedCompanyIds, companyId) ? companyId : null;
}

export function canAccessProjectIntakeCompany(
  selectedCompanyId: string | null,
  companyId: string | null | undefined,
): boolean {
  return Boolean(selectedCompanyId && companyId && selectedCompanyId === companyId);
}
