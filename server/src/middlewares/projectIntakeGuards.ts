...
import { getExplicitCompanyId } from './explicitCompanyScope';
...
const loadProjectIntake = async (req: Request) => {
  ...
  return prisma.projectIntake.findUnique({
    where: { id },
    include: { estimateRequest: true, project: true },
  });
};
...
const guard = (mode: 'view' | 'edit' | 'review') => async (
  ...
  try {
    ...
    if (!canAccessProjectIntakeCompany(selectedCompanyId, intake.project.companyId)) {
      return res.status(403).json({ error: 'Forbidden: project intake is outside the selected company' });
    }
    const scope = {
