...
const assertCompanyMatch = (
  selectedCompanyId: string,
  resourceCompanyId: string | null | undefined,
  resourceName: string,
) => {
  if (!resourceCompanyId) {
    throw httpError(`${resourceName} has no assigned company workspace`, 409);
  }
  if (resourceCompanyId !== selectedCompanyId) {
    throw httpError(`${resourceName} belongs to another company workspace`, 409);
  }
};
...
async function loadOutcome(
  ...
  assertCompanyMatch(selectedCompanyId, request.companyId, 'Estimate request');
  assertCompanyMatch(selectedCompanyId, decision.companyId, 'Commercial decision');
  if (decision.project) {
    assertCompanyMatch(selectedCompanyId, decision.project.companyId, 'Project');
  }
...
export const recordCommercialDecision = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const selectedCompanyId = getExplicitCompanyId(res);
    ...
    if (existing) {
      ...
      assertCompanyMatch(selectedCompanyId, existing.companyId, 'Commercial decision');
      return res.status(200).json(await loadOutcome(requestId, existing.id, true, selectedCompanyId));
    }
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      assertCompanyMatch(selectedCompanyId, current.companyId, 'Estimate request');
      ...
      if (parsed.data.decision === 'WON') {
        ...
        assertCompanyMatch(selectedCompanyId, owner.companyId, 'Assigned owner');
    ...
    res.status(201).json(await loadOutcome(requestId, created.decisionId, false, selectedCompanyId));
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
      ...
      if (duplicate && duplicate.estimateRequestId === String(req.params.id)) {
        const selectedCompanyId = getExplicitCompanyId(res);
        if (duplicate.companyId !== selectedCompanyId) {
          return res.status(409).json({ error: 'Commercial decision belongs to another company workspace' });
        }
        return res.status(200).json(
          await loadOutcome(duplicate.estimateRequestId, duplicate.id, true, selectedCompanyId),
        );
      }
