...
export const createEstimateRequest = async (req: Request, res: Response) => {
  try {
    const actor = (req as any).user;
    const companyId = getExplicitCompanyId(res);
    ...
    if (idempotencyKey) {
      ...
      if (existing) {
        if (existing.companyId !== companyId) {
          return res.status(409).json({ error: 'Idempotency key belongs to another company workspace' });
        }
    ...
    if (ownerId) {
      ...
      if (owner.companyId !== companyId) {
        return res.status(409).json({ error: 'Assigned owner belongs to another company workspace' });
      }
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({
        data: audit(actor.personnelId, 'ESTIMATE_REQUEST_CREATE', request.id, { requestNo, companyId }),
      });
