...
```json
{
  "id": "demo-vq-structure-023",
  "name": "[DEMO] Viet QS Team Lead 060",
  "displayName": "[DEMO] Viet QS Team Lead 060",
  "companyId": "VIET_QS",
...
```json
{
  "id": "demo-vq-structure-024",
  "name": "[DEMO] Viet QS Team Member 061",
  "displayName": "[DEMO] Viet QS Team Member 061",
  "companyId": "VIET_QS",
  "departmentId": "dept-structure-vn",
  "role": "WORKER",
  "systemRole": "WORKER",
...
## 3. 시드 파일을 수정하지 않는 이유
`fullScheduleSeed.ts` 파일에는 이미 프로젝트의 `pmId`로 `demo-vq-structure-023`이 사용되고 있으며, 작업(Task)의 `assigneeId`로 `demo-vq-structure-024`이 지정되어 있습니다. 따라서 이 아이디들을 가진 테스트 사용자만 인사 정보 JSON 파일에 추가하면, 로직 수정이나 시드 데이터 변경 없이 기존 데모 데이터와 자동으로 연동됩니다.
...
1. 헤더의 계정 Select에서 **[DEMO] CONCOST Executive 001**을 선택합니다.
...
## 7. PM 검증 기대값
- `pmId`가 `demo-vq-structure-023`인 프로젝트 표시 확인
- 프로젝트 보드 정상 접근 확인
- 프로젝트 편집 권한 동작 확인
- `ManagementSupportWidget`에서 **HR 탭 비노출** 확인
