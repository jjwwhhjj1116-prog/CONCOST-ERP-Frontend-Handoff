...
const widgetOptions: Array<{ id: WidgetId; label: string; icon: React.ElementType }> = [
  { id: 'projects', label: '참여 프로젝트', icon: FolderKanban },
  { id: 'kpi', label: 'KPI 성과', icon: Target },
  { id: 'approvals', label: '결재 현황', icon: FileCheck2 },
  { id: 'sales', label: '수주 현황', icon: CircleDollarSign },
  { id: 'tasks', label: '우선 업무', icon: CheckCircle2 },
  { id: 'schedule', label: '일정 요약', icon: CalendarDays },
];
...
export function WorkspaceWidgetPortal() {
  ...
  return (
    <section className="space-y-4" aria-label="개인화 대시보드">
      <div className="flex flex-col gap-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h2 className="text-sm font-black text-[var(--color-text-main)]">나의 워크스페이스</h2><p className="text-[11px] font-semibold text-[var(--color-text-sub)]">필요한 위젯을 선택하고 각 현황에서 바로 업무로 이동합니다.</p></div>
        </div>
        <button type="button" onClick={() => setCustomizing((value) => !value)} aria-expanded={customizing} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--cc-surface-2)] px-3 text-xs font-black text-[var(--color-text-main)] hover:border-[#ff6b00] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ff6b00]"><Settings2 className="h-4 w-4 text-[#eb6300]" />위젯 설정</button>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-12">
        {visible('projects') && <Link href="/projects" className="cc-tactile-card group min-h-[238px] p-5 xl:col-span-5" data-interactive="true">
          <div className="mb-5 flex items-center justify-between"><div><h3 className="text-sm font-black text-[var(--color-text-main)]">참여중인 프로젝트</h3><p className="mt-1 text-[10px] font-semibold text-[var(--color-text-sub)]">진행률과 납품 흐름을 한눈에 확인합니다.</p></div><ChevronRight className="h-5 w-5 text-[#eb6300] transition-transform group-hover:translate-x-1" /></div>
          <div className="overflow-hidden rounded-lg border border-[var(--color-border)]">
            <div className="grid grid-cols-[128px_repeat(5,1fr)] bg-[var(--cc-surface-2)] text-[9px] font-black text-[var(--color-text-sub)]"><span className="px-3 py-2">프로젝트</span>{['접수', 'PM', '일정', '작업', '납품'].map((step) => <span key={step} className="border-l border-[var(--color-border)] px-1 py-2 text-center">{step}</span>)}</div>
            ...
            {myProjects.length === 0 && <div className="px-4 py-10 text-center text-xs font-semibold text-[var(--color-text-sub)]">참여중인 프로젝트가 없습니다.</div>}
          </div>
        </Link>}

        {visible('kpi') && <Link href="/tasks/my" className="cc-tactile-card group min-h-[238px] p-5 xl:col-span-3" data-interactive="true">
          <div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-black text-[var(--color-text-main)]">KPI 성과</h3><Target className="h-4 w-4 text-[#eb6300]" /></div>
          <Donut segments={[completedTasks, reviewTasks, Math.max(0, myTasks.length - completedTasks - reviewTasks)]} center={`${kpiRate}%`} label="업무 달성률" />
          <div className="mt-3 grid grid-cols-3 text-center text-[9px] font-bold text-[var(--color-text-sub)]"><span><b className="block text-sm text-emerald-600">{completedTasks}</b>완료</span><span><b className="block text-sm text-orange-500">{reviewTasks}</b>검토</span><span><b className="block text-sm text-red-500">{delayedTasks}</b>지연</span></div>
        </Link>}
        ...
        {visible('approvals') && <Link href="/approvals" className="cc-tactile-card group min-h-[238px] p-5 xl:col-span-4" data-interactive="true">
          <div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-black text-[var(--color-text-main)]">결재 현황</h3><FileCheck2 className="h-4 w-4 text-[#eb6300]" /></div>
          <Donut segments={[approved, pending, rejected]} center={`${approved}/${visibleApprovals.length}`} label="승인 문서" />
        ...
        {visible('tasks') && <Link href="/tasks/my" className="cc-tactile-card group min-h-[196px] p-5 xl:col-span-5" data-interactive="true">
          <div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-black text-[var(--color-text-main)]">우선 업무</h3><ChevronRight className="h-4 w-4 text-[#eb6300]" /></div>
          <div className="divide-y divide-[var(--color-border)]">{myTasks.filter((task) => task.status !== 'DONE').slice(0, 4).map((task) => <div key={task.id} className="flex items-center gap-3 py-2.5"><span className={`h-2 w-2 rounded-full ${task.priority === 'URGENT' ? 'bg-red-500' : 'bg-[#ff8a1f]'}`} /><span className="min-w-0 flex-1 truncate text-[11px] font-bold text-[var(--color-text-main)]">{task.title}</span><span className="text-[9px] font-semibold text-[var(--color-text-sub)]">{task.dueDate || '기한 없음'}</span></div>)}{myTasks.filter((task) => task.status !== 'DONE').length === 0 && <p className="py-10 text-center text-xs text-[var(--color-text-sub)]">우선 처리할 업무가 없습니다.</p>}</div>
        </Link>}

        {visible('schedule') && <Link href="/calendar" className="cc-tactile-card group min-h-[196px] p-5 xl:col-span-3" data-interactive="true">
          <div className="mb-4 flex items-center justify-between"><h3 className="text-sm font-black text-[var(--color-text-main)]">오늘 일정</h3><CalendarDays className="h-5 w-5 text-[#eb6300]" /></div>
          <strong className="font-mono text-4xl font-black text-[var(--color-text-main)]">{todaySchedules.length}</strong><span className="ml-1 text-xs font-bold text-[var(--color-text-sub)]">건</span>
          <p className="mt-3 text-[11px] font-semibold leading-5 text-[var(--color-text-sub)]">등록한 회의, 휴가와 프로젝트 일정을 캘린더에서 확인할 수 있습니다.</p>
          <div className="mt-4 flex gap-2"><span className="rounded-md bg-[#eef2ff] px-2 py-1 text-[9px] font-black text-[#405bb0]"><Mail className="mr-1 inline h-3 w-3" />업무 연결</span><span className="rounded-md bg-orange-50 px-2 py-1 text-[9px] font-black text-orange-700">일정 보기</span></div>
        </Link>}
      </div>
    </section>
  );
