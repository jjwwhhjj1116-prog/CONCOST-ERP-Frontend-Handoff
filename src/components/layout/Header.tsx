...
export function Header() {
  ...
  const { isDarkMode, toggleDarkMode, brandWorkspace } = useUiStore();
  ...
  const sectionLabel = pathname.startsWith('/projects') ? '프로젝트' : pathname.startsWith('/schedules') ? '일정관리' : pathname.startsWith('/approvals') ? '전자결재' : pathname.startsWith('/mail') ? '전자메일' : pathname.startsWith('/tasks') ? '할일' : pathname.startsWith('/drive') ? '드라이브' : pathname.startsWith('/board') ? '게시판' : pathname.startsWith('/organization') ? '조직도' : pathname.startsWith('/settings') ? '설정' : 'HOME';
        ...
        <div className="relative">
          ...
          {profileOpen && (
            <div className="absolute right-0 top-[calc(100%+10px)] w-60 overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-2 shadow-[var(--cc-shadow-3)]">
              ...
              <Link href="/settings/profile" onClick={() => setProfileOpen(false)} className="flex min-h-10 items-center gap-2 rounded-xl px-3 text-xs font-bold text-[var(--color-text-sub)] hover:bg-[var(--cc-surface-2)] hover:text-[var(--color-text-main)]"><Settings2 className="h-4 w-4" /> 프로필 사진</Link>
