...
import { AlertTriangle, Building2, ClipboardPen, Save, Undo2, UserRound } from 'lucide-react';
...
const sectionClass = 'scroll-mt-24 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-[0_8px_22px_rgba(15,23,42,.06)] sm:p-5';
const inputClass = 'min-h-10 w-full rounded border border-[var(--color-border)] bg-[var(--color-surface)] px-3 text-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-[var(--color-text-sub)]';
...
export function EstimateRequestProfileEditor({ request, mode, disabled = false, busy, onSave, onCancel }: Props) {
  ...
  const editable = !disabled && (mode === 'EDIT' || mode === 'CORRECTION');
  ...
  return <>
    <nav aria-label="견적 의뢰 상세 단계" className="sticky top-0 z-20 grid grid-cols-2 gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)]/95 p-3 shadow-sm backdrop-blur sm:grid-cols-4 xl:grid-cols-7">
      {STEPS.map(([number, label]) => <button key={number} type="button" onClick={() => document.getElementById(`estimate-step-${number}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="min-h-12 rounded border border-[var(--color-border)] bg-[var(--color-surface)] px-2 text-left transition hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]">
    ...
    {mode !== 'VIEW' && <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-orange-300 bg-orange-50 px-4 py-3 text-sm text-orange-950">
      <div><strong>{mode === 'CORRECTION' ? '접수 전 정보 정정 중' : mode === 'SAVING' ? '저장 중' : '수정 중'}</strong><span className="ml-2 text-xs">변경 후 저장하거나 취소할 수 있습니다.</span></div>
      <span className="rounded-full border border-orange-300 bg-white px-2.5 py-1 text-xs font-black">{mode}</span>
    </div>}
    ...
    <section id="estimate-step-01" className={sectionClass}>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <ProfileField disabled={!editable} label="입찰/실행" value={profile.executionType} onChange={(value) => set('executionType', value)} field="executionType" />
        <MemoryField disabled={!editable} label="건물용도" required value={profile.usage} onChange={(value) => set('usage', value)} field="usage" />
        <ProfileField disabled={!editable} label="연면적 m²" value={profile.areaM2} onChange={(value) => set('areaM2', value)} field="areaM2" inputMode="decimal" />
        <ProfileField disabled={!editable} label="연면적 평" value={profile.areaPy} onChange={(value) => set('areaPy', value)} field="areaPy" inputMode="decimal" />
        <ProfileField disabled={!editable} label="동수" value={profile.buildingCount} onChange={(value) => set('buildingCount', value)} field="buildingCount" />
        <ProfileField disabled={!editable} label="지하층" value={profile.basementFloors} onChange={(value) => set('basementFloors', value)} field="basementFloors" />
        <ProfileField disabled={!editable} label="지상층" value={profile.groundFloors} onChange={(value) => set('groundFloors', value)} field="groundFloors" />
        <ProfileField disabled={!editable} label="층수 메모" value={profile.floors} onChange={(value) => set('floors', value)} field="floors" />
        <ProfileField disabled={!editable} label="입찰일" value={profile.bidDate} onChange={(value) => set('bidDate', value)} field="bidDate" type="date" />
        <ProfileField disabled={!editable} label="납기" required value={profile.firstDelivery} onChange={(value) => set('firstDelivery', value)} field="firstDelivery" type="date" />
        <MemoryField disabled={!editable} label="단가작업여부" value={profile.unitWork} onChange={(value) => set('unitWork', value)} field="unitWork" />
        <MemoryField disabled={!editable} label="견적종류" value={profile.estimateType} onChange={(value) => set('estimateType', value)} field="estimateType" />
        <label className="text-sm sm:col-span-2 xl:col-span-4"><span className="mb-1 block font-bold">업무범위 <b className="text-red-600">필수</b></span><textarea disabled={!editable} data-estimate-field="scope" rows={3} value={profile.scope} onChange={(event) => set('scope', event.target.value)} className={`${requiredClass} py-2`} /></label>
        <div data-estimate-field="targetUnitIds" tabIndex={-1} className="sm:col-span-2 xl:col-span-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]">
          <ProjectExecutionUnitSelector value={profile.targetUnitIds} primaryUnitId={profile.primaryUnitId} disabled={!editable} onChange={(targetUnitIds: ProjectExecutionUnitId[], primaryUnitId) => setProfile((current) => ({ ...current, targetUnitIds, primaryUnitId }))} />
        </div>
      </div>
    </section>
    ...
    <section id="estimate-step-02" className={sectionClass}>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        <ProfileField disabled={!editable} label="담당자" value={profile.contact} onChange={(value) => set('contact', value)} field="contact" />
        <ProfileField disabled={!editable} label="부서/직급" value={profile.contactDepartment} onChange={(value) => set('contactDepartment', value)} field="contactDepartment" />
        <ProfileField disabled={!editable} label="전화" value={profile.phone} onChange={(value) => set('phone', value)} field="phone" />
        <ProfileField disabled={!editable} label="이메일" value={profile.email} onChange={(value) => set('email', value)} field="email" type="email" />
      </div>
    </section>
...
function ProfileField({ label, value, onChange, field, required = false, type = 'text', inputMode, disabled = false }: { label: string; value: string; onChange: (value: string) => void; field: keyof EstimateRequestProfile; required?: boolean; type?: string; inputMode?: 'decimal'; disabled?: boolean }) {
  return <label className="text-sm"><span className="mb-1 block font-bold">{label} {required && <b className="text-red-600">필수</b>}</span><input disabled={disabled} data-estimate-field={field} type={type} inputMode={inputMode} value={value} onChange={(event) => onChange(event.target.value)} className={required ? requiredClass : inputClass} /></label>;
}

function MemoryField({ label, value, onChange, field, required = false, disabled = false }: { label: string; value: string; onChange: (value: string) => void; field: 'vendor' | 'workCategory' | 'usage' | 'unitWork' | 'estimateType'; required?: boolean; disabled?: boolean }) {
  return <label className="text-sm"><span className="mb-1 block font-bold">{label} {required && <b className="text-red-600">필수</b>}</span><InputHistoryInput disabled={disabled} data-estimate-field={field} moduleKey="estimate-request" fieldKey={field} value={value} onChange={onChange} className={required ? requiredClass : inputClass} /></label>;
}
