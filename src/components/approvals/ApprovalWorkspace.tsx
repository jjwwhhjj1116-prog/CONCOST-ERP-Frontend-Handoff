    ...
    <div className="space-y-5">
      <header className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <ActionButtonGroup label="전자결재 주요 작업">
            <SemanticActionButton variant="view" icon={<UsersRound className="h-4 w-4" />} tooltip={copy.lineManager} onClick={() => setLineManagerOpen(true)}>{copy.lineManager}</SemanticActionButton>
            <SemanticActionButton variant="edit" icon={<Settings2 className="h-4 w-4" />} tooltip={copy.delegation} onClick={() => setMessage({ kind: 'info', text: copy.permission })}>{copy.delegation}</SemanticActionButton>
            <SemanticActionButton variant="primary" icon={<FilePlus2 className="h-4 w-4" />} tooltip={copy.write} onClick={() => setCatalogOpen(true)}>{copy.write}</SemanticActionButton>
          </ActionButtonGroup>
        </div>
      </header>
        ...
        <div className="divide-y divide-[var(--color-border)]">
          {queue.length === 0 ? <div className="flex min-h-56 flex-col items-center justify-center text-center"><FileCheck2 className="mb-3 h-9 w-9 text-[#b6c2df]" /><strong className="text-sm font-black">{copy.empty}</strong><span className="mt-1 text-xs text-[var(--color-text-sub)]">{copy.emptyDescription}</span></div> : queue.map((request) => {
            ...
            return <article key={request.id} className="grid gap-4 p-5 hover:bg-[#f8faff] dark:hover:bg-white/[.03] lg:grid-cols-[minmax(0,1fr)_minmax(280px,420px)_auto] lg:items-center">
              ...
              <ActionButtonGroup label={`${request.title} 작업`} className="lg:justify-end">
                {isActionable(request) && <>
                  <SemanticActionButton size="sm" variant="success" icon={<Check className="h-3.5 w-3.5" />} tooltip={copy.approve} onClick={() => setDecision({ request, action: 'APPROVE' })}>{copy.approve}</SemanticActionButton>
                  <SemanticActionButton size="sm" variant="warning" icon={<RotateCcw className="h-3.5 w-3.5" />} tooltip={copy.requestChanges} onClick={() => setDecision({ request, action: 'REQUEST_CHANGES' })}>{copy.requestChanges}</SemanticActionButton>
                  <SemanticActionButton size="sm" variant="reject" icon={<X className="h-3.5 w-3.5" />} tooltip={copy.reject} onClick={() => setDecision({ request, action: 'REJECT' })}>{copy.reject}</SemanticActionButton>
                </>}
                {canRecall && <SemanticActionButton size="sm" variant="archive" tooltip={copy.recall} onClick={() => recall(request)}>{copy.recall}</SemanticActionButton>}
                <SemanticActionButton size="sm" variant="view" icon={<ChevronRight className="h-3.5 w-3.5" />} tooltip={copy.view} onClick={() => setDetail(request)}>{copy.view}</SemanticActionButton>
              </ActionButtonGroup>
            </article>;
          })}
        </div>
