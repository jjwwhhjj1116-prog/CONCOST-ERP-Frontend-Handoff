...
export function ApprovalLineManager({ locale, companyId, currentUser, users, formType, initialSteps, onApply, onClose }: Props) {
  ...
  return (
    <ResponsiveDialogShell
      ...
      onClose={onClose}
      footer={<div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={onClose} className="min-h-10 rounded-lg border border-[var(--color-border)] px-4 text-xs font-black">{copy.cancel}</button><button type="button" onClick={save} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[var(--color-primary)] px-4 text-xs font-black text-[var(--color-primary)]"><Save className="h-4 w-4" />{copy.save}</button><button type="button" onClick={() => onApply(draft)} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-[var(--color-primary)] px-4 text-xs font-black text-white"><Check className="h-4 w-4" />{copy.apply}</button></div>}
    >
      <div className="grid min-w-0 gap-5 lg:grid-cols-[300px_minmax(0,1fr)]">
        <aside className="min-w-0 border border-[var(--color-border)] bg-[var(--cc-surface-2)] p-3">
          <div className="flex items-center justify-between gap-2"><h3 className="text-xs font-black">{copy.saved}</h3><button type="button" onClick={() => setDraft(createLineDefinition({ companyId, ownerId: currentUser.id, departmentId: currentUser.departmentId, name: '새 결재선', formType, steps: initialSteps }))} className="inline-flex min-h-9 items-center gap-1 rounded-lg bg-[var(--color-primary)] px-3 text-[10px] font-black text-white"><Plus className="h-3.5 w-3.5" />{copy.newLine}</button></div>
        ...
        <section className="min-w-0 space-y-4">
          <div className="grid gap-3 rounded-xl border border-[var(--color-border)] bg-[var(--cc-surface-2)] p-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="sm:col-span-2 xl:col-span-4 flex flex-wrap items-center gap-2 text-[10px] font-bold text-[var(--color-text-sub)]"><span>{copy.steps} {draft.steps.length}</span><span>{copy.author} {draft.ownerId}</span><span>{copy.usage} {draft.usageCount}</span><span>{copy.version} {draft.version}</span><button type="button" onClick={() => { const id = copyLine(draft.id, currentUser.id); if (id) setMessage(copy.copy); }} disabled={!savedLines.some((line) => line.id === draft.id)} className="ml-auto inline-flex min-h-8 items-center gap-1 border border-[var(--color-border)] px-2 disabled:opacity-40"><Copy className="h-3 w-3" />{copy.copy}</button><button type="button" onClick={() => setDefaultLine(draft.id, currentUser.id)} disabled={!savedLines.some((line) => line.id === draft.id)} className="inline-flex min-h-8 items-center gap-1 border border-[var(--color-border)] px-2 disabled:opacity-40"><Star className="h-3 w-3" />{copy.setDefault}</button><button type="button" onClick={() => { if (deleteLine(draft.id, currentUser.id)) setDraft(createLineDefinition({ companyId, ownerId: currentUser.id, name: '새 결재선', formType, steps: initialSteps })); else setMessage(copy.warning); }} disabled={!savedLines.some((line) => line.id === draft.id)} className="inline-flex min-h-8 items-center gap-1 border border-red-200 px-2 text-red-700 disabled:opacity-40"><Trash2 className="h-3 w-3" />{copy.remove}</button></div>
          </div>

          <div className="flex items-center justify-between"><h3 className="flex items-center gap-2 text-sm font-black"><UsersRound className="h-4 w-4 text-[var(--color-primary)]" />{copy.steps}</h3><button type="button" onClick={addStep} className="inline-flex min-h-9 items-center gap-1 rounded-lg border border-[var(--color-primary)] px-3 text-[10px] font-black text-[var(--color-primary)]"><Plus className="h-3.5 w-3.5" />{copy.addStep}</button></div>
