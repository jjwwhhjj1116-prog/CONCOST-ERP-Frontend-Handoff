...
export type ProjectWorkflowTab = typeof PROJECT_WORKFLOW_TABS[number];
export type ProjectConnectedWorkspace = 'APPROVAL' | 'DRIVE' | 'AI';
...
export const getProjectConnectedWorkspaceHref = (projectId: string, workspace: ProjectConnectedWorkspace) => {
  const path = workspace === 'APPROVAL' ? '/approvals' : workspace === 'DRIVE' ? '/drive' : '/ai-assistant';
