...
export const buildLocalWonPipeline = ({
  ...
  const updatedRequest: EstimateRequest = {
    ...request,
    status: 'WON',
    worklistState: 'TRANSFERRED_TO_INTAKE',
    transferredToIntakeAt: timestamp,
    projectId,
    projectIntakeId: intakeId,
    commercialDecisionId: decisionId,
    targetUnitIds,
    ...
    histories: [{
      ...
      estimateRequestId: request.id,
      action: 'ESTIMATE_REQUEST_TRANSFERRED_TO_INTAKE',
      fromStatus: request.status,
      toStatus: 'WON',
      changes: JSON.stringify({
        before: { worklistState: request.worklistState || 'ACTIVE', projectIntakeId: request.projectIntakeId || null },
        after: { worklistState: 'TRANSFERRED_TO_INTAKE', projectIntakeId: intakeId, projectId },
        decisionId,
        targetUnitIds,
        primaryUnitId,
        estimateSubmissionId,
