...
function IntakePageContent() {
  ...
  const [activeTab, setActiveTab] = useState<IntakeTab>('PROJECT_INTAKE');
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      const requestedTab = searchParams.get('tab') as IntakeTab | null;
      if (requestedTab && ['CLIENT_ORDER', 'INTERNAL_DEVELOPMENT', 'PROJECT_INTAKE'].includes(requestedTab)) {
        setActiveTab(requestedTab);
      } else if (searchParams.has('intakeId')) {
        setActiveTab('PROJECT_INTAKE');
      } else if (searchParams.has('requestId')) {
...
export default function IntakePage() {
  return (
    <Suspense fallback={<div role="status" className="flex justify-center p-8"><LoaderCircle aria-hidden="true" className="size-5 animate-spin" /></div>}>
      <IntakePageContent />
    </Suspense>
  );
}
