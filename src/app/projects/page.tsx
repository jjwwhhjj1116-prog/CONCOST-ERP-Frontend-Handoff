...
export default function ProjectBoardPage() {
  ...
  useEffect(() => {
    ...
    if (!projectId) return;
    const timer = window.setTimeout(() => setWorkflowTarget({
      projectId,
      tab: requestedTab && PROJECT_WORKFLOW_TABS.includes(requestedTab) ? requestedTab : 'OVERVIEW',
    }), 0);
        ...
        <section className="relative overflow-hidden rounded-[26px] border border-white/80 bg-[linear-gradient(135deg,#fff8ee_0%,#f7fbf8_48%,#eef3fb_100%)] p-5 text-slate-900 shadow-[0_24px_60px_rgba(74,78,91,.13),0_2px_8px_rgba(74,78,91,.08)] sm:p-7">
          ...
          <div className="flex flex-col gap-6 2xl:flex-row 2xl:items-end 2xl:justify-between">
            ...
            <div className="relative z-10 grid grid-cols-2 gap-2.5 sm:grid-cols-5">
              {[
                { label: '전체', value: projectStats.total, icon: Layers3, tone: 'border-slate-200/90 bg-white/90 text-slate-700', iconTone: 'bg-slate-100 text-slate-600' },
                { label: '진행 중', value: projectStats.active, icon: Activity, tone: 'border-sky-200/80 bg-[#eff8ff]/95 text-[#17618f]', iconTone: 'bg-[#dcefff] text-[#2383bd]' },
                { label: '14일 내 납품', value: projectStats.dueSoon, icon: Clock3, tone: 'border-amber-200/90 bg-[#fff8e8]/95 text-[#93620c]', iconTone: 'bg-[#ffebba] text-[#b47808]' },
                { label: '완료', value: projectStats.completed, icon: CheckCircle2, tone: 'border-emerald-200/90 bg-[#edfaf4]/95 text-[#13705a]', iconTone: 'bg-[#d4f3e5] text-[#16856a]' },
                { label: '긴급', value: projectStats.urgent, icon: AlertTriangle, tone: 'border-rose-200/90 bg-[#fff1f3]/95 text-[#a9354b]', iconTone: 'bg-[#ffdfe4] text-[#c3485e]' },
              ].map((stat) => {
                const Icon = stat.icon;
                return <div key={stat.label} className={`min-w-[116px] rounded-2xl border ${stat.tone} p-3 shadow-[0_8px_18px_rgba(74,78,91,.08),inset_0_1px_0_rgba(255,255,255,.8)] backdrop-blur-sm`}><div className="flex items-center justify-between gap-3"><span className="text-[10px] font-black tracking-[-.01em]">{stat.label}</span><span className={`grid h-7 w-7 place-items-center rounded-xl ${stat.iconTone}`}><Icon className="h-3.5 w-3.5" /></span></div><strong className="mt-2 block text-2xl font-black tracking-tight">{stat.value}</strong></div>;
              })}
            </div>
          </div>
        </section>
