...
export default function MyTasksPage() {
  ...
  const taskBoundary = getFrontendModuleBoundary('TASK', {
    adapterReady: false,
  });
  ...
  const handleRequestCompletion = async () => {
    if (selectedTask && memo) {
      const result = await executeFrontendMutation(taskBoundary, {
        simulate: () => requestTaskCompletion(selectedTask.id, memo),
      });
  ...
  const handleReviewCompletion = async (taskId: string, approved: boolean) => {
    const result = await executeFrontendMutation(taskBoundary, {
      simulate: () => reviewTaskCompletion(taskId, approved),
    });
    setFeedback({ kind: result.kind, message: result.message });
  };
  ...
  return (
    <div className="w-full px-6 mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <ModuleHandoffPanel
        module="TASK"
        description={{
          ko: '개인·팀·프로젝트 할 일과 체크리스트·댓글·리마인더를 하나의 Backend 계약으로 연결합니다.',
          vi: 'Kết nối công việc cá nhân, nhóm, dự án, checklist, bình luận và nhắc việc với một hợp đồng Backend.',
          en: 'Connect personal, team, and project tasks with checklists, comments, and reminders through one Backend contract.',
        }}
      />
      {feedback && (
        <div
