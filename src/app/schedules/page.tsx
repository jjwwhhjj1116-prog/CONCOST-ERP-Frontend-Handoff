...
import { Activity, CalendarRange, CheckCircle2, Clock3, UserPlus, UsersRound } from 'lucide-react';
...
export default function SchedulesPage() {
  ...
  const visibleUsers = scopedUsers.filter(u => {
    ...
    if (!showAllUsers) {
      if (u.id === 'demo-cc-admin-001' || u.id === 'demo-cc-executive-002') return false;
      ...
      <section className="overflow-hidden rounded-[24px] bg-[linear-gradient(120deg,#0f766e_0%,#155e75_48%,#273e7a_100%)] p-5 text-white shadow-[0_22px_55px_rgba(21,94,117,.20)] sm:p-7">
        <div className="flex flex-col gap-6 2xl:flex-row 2xl:items-end 2xl:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
            ...
            ].map((stat) => { const Icon = stat.icon; return <div key={stat.label} className="min-w-[126px] rounded-2xl border border-white/15 bg-white/12 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,.12)] backdrop-blur-sm"><div className="flex items-center justify-between"><span className="text-[10px] font-black text-cyan-50/85">{stat.label}</span><Icon className="h-4 w-4 text-white/80" /></div><strong className="mt-2 block text-2xl font-black">{stat.value}</strong></div>; })}
          </div>
        </div>
      </section>
            ...
            <table className="w-full min-w-[1000px] border-separate border-spacing-0 [&_td]:border-[var(--color-border)] [&_th]:border-[var(--color-border)]">
              ...
              <tbody>
                {scopedProjects.map(project => {
                  ...
                  const projectTasks = approvedTasks.filter((task) => task.projectId === project.id && task.assigneeId);
                  const projectPeople = Array.from(new Set([project.pmId, ...projectTasks.map((task) => task.assigneeId)].filter(Boolean)))
                    .map((id) => users.find((user) => user.id === id))
                    .filter((user): user is NonNullable<typeof user> => Boolean(user));
                  const workScopes = Array.from(new Set(projectTasks.map((task) => task.scopeName || task.title).filter(Boolean))).slice(0, 3);
                      ...
                      <td className="sticky left-0 bg-[var(--color-surface)] border-b border-r-2 p-3 text-sm font-bold text-[var(--color-text-main)] z-10 shadow-[1px_0_0_0_#e5e7eb]">
                        ...
                        <div className="mt-2 flex items-center justify-between gap-2">
                          <div className="flex -space-x-1.5">
                            {projectPeople.slice(0, 6).map((person) => <span key={person.id} title={`${getUserDisplayName(person)} · ${person.teamName || person.departmentName || ''}`} className={`grid h-7 w-7 place-items-center rounded-full border-2 border-white text-[9px] font-black text-white shadow-sm ${getAvatarTone(person.id)}`}>{getUserDisplayName(person).slice(0, 1)}</span>)}
                            {projectPeople.length > 6 && <span className="grid h-7 w-7 place-items-center rounded-full border-2 border-white bg-slate-600 text-[9px] font-black text-white">+{projectPeople.length - 6}</span>}
                          </div>
                          <button type="button" onClick={() => setActiveTab('PM_MANAGEMENT')} className="inline-flex min-h-7 items-center gap-1 rounded-md border border-[var(--color-border)] px-2 text-[9px] font-black text-[var(--color-text-sub)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]"><UserPlus className="h-3 w-3" />인력·공종 배정</button>
                        </div>
                        {workScopes.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{workScopes.map((scope) => <span key={scope} className="rounded-full bg-indigo-50 px-2 py-0.5 text-[9px] font-bold text-indigo-700">{scope}</span>)}</div>}
                      </td>
                      {daysArray.map(d => {
                        const targetDate = new Date(year, month, d).setHours(0,0,0,0);
